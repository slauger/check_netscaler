...
sub check_state
{
        ...
        # a global counter is pretty useless for a single object (also für PARTIAL-UP for servicegroups)
        if (!defined($plugin->opts->objectname)) {
                foreach my $key (keys %counter) {
                        $plugin->add_message(OK, $counter{$key} . ' ' . $key . ';');

                        # replay '-' with '_' for rrd
                        my $rrdlabel = join( "_", split("-", $key) );
...
sub check_interfaces
{
        ...
        foreach my $interface (@{$response->{'Interface'}}) {
                ...
                # RRD doesn't like '." in lables -> replaced by '_'
                $plugin->add_perfdata(
                ...
                $plugin->add_perfdata(
                        label    => "\'".$interface->{'devicename'} . "_rxerrors'",
                        value    => $interface->{'rxerrors'}.'c'
                );
                $plugin->add_perfdata(
                        label    => "\'".$interface->{'devicename'} . "_txerrors'",
